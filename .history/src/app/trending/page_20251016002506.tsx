...
export default function TrendingPage() {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-pink-100 via-purple-100 to-pink-50 overflow-hidden">
      ...
      {/* 修改后的英雄区 - 轮播显示 */}
      <section
        className={`relative z-10 flex flex-col md:flex-row items-center justify之间 px-16 py-20 transition-all duration-300 ${
      ...
      >
        <div className="w-full md:w-1/2 mb-10 md:mb-0 relative">
          ...
          <button
            onClick={prevHero}
            className="absolute left-2 top-1/2 transform -translate-y-1/2 bg白/80 hover:bg白 p-2 rounded-full shadow-lg transition-all duration-300"
          >
            <ChevronLeft className="w-6 h-6 text-purple-600" />
          </button>
          <button
            onClick={nextHero}
            className="absolute right-2 top-1/2 transform -translate-y-1/2 bg白/80 hover:bg白 p-2 rounded-full shadow-lg transition-all duration-300"
          >
                ...
                <motion.div
                  ...
                  className={`relative p-4 rounded-2xl shadow-lg cursor-pointer transition-all duration-300 ${
                    isActive
                      ...
                      : "bg-white/70 text-gray-700 hover:bg白/90"
                  }`}
      ...
      <section
        className={`relative z-10 px-10 py-12 bg白/50 backdrop-blur-lg rounded-t-3xl transition-all duration-300 ${
          sidebarCollapsed ? "ml-20" : "ml-80"
        }`}
      >
        <h3 className="text-2xl font-bold text紫-700 mb-8 text-center">
          加密货币保险产品
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {allEvents.map((product, i) => (
            <div
              key={i}
              className="bg白/70 rounded-2xl shadow-md border border白/30 overflow-hidden"
            >
              {/* 产品图片 */}
